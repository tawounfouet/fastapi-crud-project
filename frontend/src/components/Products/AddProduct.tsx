...
import { type ApiError, DemoService, type ProductCreate } from "@/client"
import {
  DialogActionTrigger,
  DialogBody,
  DialogCloseTrigger,
...
const AddProduct = ({ disabled }: AddProductProps) => {
  ...
  const mutation = useMutation({
    mutationFn: (data: ProductCreate) =>
      DemoService.createProduct({ requestBody: data }),
    onSuccess: () => {
          ...
          <DialogBody>
            <VStack gap={4}>
              ...
              <Field.Root>
                ...
                <Switch.Root defaultChecked={true} {...register("is_active")}>
                  <Switch.Control>
                    <Switch.Thumb />
                  </Switch.Control>
                </Switch.Root>
